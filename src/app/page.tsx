          ...
          {featuredPrograms.map((item, index) => {
            ...
            return (
              <MotionArticle key={item.title} delay={index * 0.1} className={`group flex min-h-full flex-col overflow-hidden rounded-xl border border-white/12 bg-surface-white p-3 text-brand-navy shadow-2xl shadow-brand-navy/20 transition hover:-translate-y-1 sm:p-4 ${index === 2 ? "md:col-span-2 xl:col-span-1" : ""}`}>
                ...
                <div className="mt-5 flex flex-wrap gap-2.5"><span className="inline-flex min-h-10 items-center rounded-md bg-brand-red px-3 text-sm font-extrabold text-white">{item.badge}</span><span className="inline-flex min-h-10 items-center rounded-md border border-brand-navy/12 bg-surface-cream px-3 text-sm font-extrabold">{item.id === "clases-grupales" ? "Online" : item.format}</span></div>
